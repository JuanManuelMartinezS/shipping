...
public interface Service<T> {
  ...
  /**
   * Crea una instancia JSONObject a partir de strJson y con ella una instancia
   * java.
   * Si la instancia ya existe en la lista, se genera un error, si no, se agrega a
   * ésta
   ...
   * @return Un JSONObject que corresponde al elemento encontrado en la posición
   *         dada.
   */
  ...
  /**
   * Si hay acceso al archivo devuelve un JSONObject con la estructura {
   * "message": "ok", "data": [...]},
   * siendo "data" el array de objetos JSON recuperados del archivo. Si no hay
   * acceso se devuleve un
  ...
  /**
   * Restaura la lista de instancias Java a partir del archivo JSON
   * correspondiente
   * 
   * @return Una lista creada a partir de los elementos recuperados del archivo
   *         JSON.
   * @throws Exception
   */
  ...
  /**
   * Busca una instancia en la lista, según el ID único dado como primer argumento
   * y le modifica
  ...
  /**
   * Elimina el ultimo estado a del envio a partir del numero de guia
   * @param numGuide
   * @return el json object actualizado sin el ultimo estado
   * @throws Exception
   */
  public JSONObject removeStatus(String numGuide) throws Exception;
  /**
   * Dado un ID, busca una instancia en la lista y la elimina sólo si no está
   * asociada
  ...
  /**
   * Devuelve un objeto de tipo T, luego de verificar que los datos dados en
   * strJson sean válidos para crear un objeto de tipo T
   * 
   * @param strJson Un JSON con los datos necesarios y válidos para crear una
   *                instancia de T
   ...
   * @return Una nueva instancia creada a partir de current pero con los datos
   *         actualizados según newData
   */
  public T getUpdated(JSONObject newData, T current) throws Exception;

  /**
   * Devuelve un objeto json con la estructura { "size" : totalRegistros,
   * "message" : "ok" }
   * siendo totalRegistros un valor entero que indica el número de registros de la
   * lista
   * 
   * @return
   */
